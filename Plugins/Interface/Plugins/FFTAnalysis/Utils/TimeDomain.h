...
inline std::vector<double> computeTWIntegral(std::vector<double> waveform, double dt, std::vector<double> coeff){
    std::vector<double> integralWaveform;

    //waveform = FFTConvolve(waveform, coeff);

    double scalar = 9810.;
    //std::transform(waveform.begin(), waveform.end(), waveform.begin(),
    //[scalar](double element) { return element * scalar; });

    //waveform = convolve(waveform, coeff);
...
*/
    ...
    return section;
    //return dataResult;
    //return convolution(integralWaveform, coeff);
}

inline void computeIntegralHPFilter(double rate, std::vector<double>& coeff){
    double fc = 5.;
    double bt = 2.;
